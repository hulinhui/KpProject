...
class KpUploadFile:
    ...
    def get_exam_data(self, org_info):
        """
        生成上传文件所需参数
        :param org_info: 机构信息
        :return: 上传接口参数
        ...
        exam_info = exam.search_exam(org_info)
        if not exam_info: return upload_data
        if self.upload_type in ['stu', 'tea', 'score', 'lin_tea']:
            data = exam.search_paper(*exam_info)
            upload_data = {'absent': 0, 'override': 'false', 'entrance': 1,
                           **data} if self.upload_type == 'score' else data
        if self.upload_type == 'lin_stu':
            upload_data = exam.exam_detail_query(exam_info)
            CreateTempStu = LazyModule('QuestionCard.KpRequest.CreateTempStu')
            CreateTempStu.GenerateExcelStu().run('普通模式')
        return upload_data

    def get_upload_info(self, org_info):
        """
        获取上传所需数据
        :param org_info    机构信息
        ...
        upload_data = self.get_exam_data(org_info)
        upload_item = {
    ...
    def run(self):
        ...
        """
        # 登录
        org_info = self.login.get_login_token(keys=['userId', 'orgType', 'roleList'])
        # 获取上传接口所需data
        upload_tuple = self.get_upload_info(org_info)
        # 判断考试相关参数是否有效
