...
default_formats = {
    # 终端输出格式
    'color_format': '%(log_color)s%(asctime)s-%(filename)s-[line:%(lineno)d]-%(levelname)s-[日志信息]: %(message)s',
    # 日志输出格式
    'log_format': '%(asctime)s-%(filename)s-[line:%(lineno)d]-%(levelname)s-[日志信息]: %(message)s'
}
...
class HandleLog:
    ...
    def __init__(self):
        ...
        self.log_path = self.__init_logpath()
    ...
    def __set_color_handle(self, console_handle, level=logging.DEBUG):
        """
